...
- **SNS topic idempotency**: `createTopic` in `snsStore.ts` returns the existing topic when called with the same name and matching attributes and tags. Bidirectional attribute comparison — checks both new→existing and existing→new attribute keys when attributes are provided. Throws `SnsError` when attributes or tags differ. Omitting attributes on a repeat call is treated as "no opinion" (returns existing topic without conflict).
...
- **S3 CopyObject metadata directive**: CopyObject respects the `x-amz-metadata-directive` header. `"COPY"` (default) preserves source object metadata. `"REPLACE"` uses metadata from request headers, allowing metadata to be cleared or overwritten.

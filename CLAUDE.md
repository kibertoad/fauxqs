...
- **Programmatic API**: `FauxqsServer` exposes `createQueue()`, `createTopic()`, `subscribe()`, `createBucket()`, `setup()`, `reset()`, `purgeAll()`, and `inspectQueue()` for state management and debugging without going through the SDK. `buildApp` accepts an optional `stores` parameter to use pre-created store instances.

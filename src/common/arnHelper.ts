...
export function sqsQueueArn(queueName: string): string {
  return `arn:aws:sqs:${DEFAULT_REGION}:${DEFAULT_ACCOUNT_ID}:${queueName}`;
}

export function snsTopicArn(topicName: string): string {
  return `arn:aws:sns:${DEFAULT_REGION}:${DEFAULT_ACCOUNT_ID}:${topicName}`;
}

export function snsSubscriptionArn(topicName: string, id: string): string {
